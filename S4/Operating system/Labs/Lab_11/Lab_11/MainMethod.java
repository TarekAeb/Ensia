...
// Synch.timeSim is code provided to allow your simulation
      ...
      try {
        ...
        // throw new InterruptedException();
          while (true) {
              // Southbound green
              Synch.mutex.acquire();
              Synch.currentLight = Synch.SOUTHBOUND_GREEN;
              System.out.println("At time " + Synch.timeSim.curTime() + " Southbound light is GREEN.");
              int numCarsToRelease = Synch.southboundCarCount;
              Synch.southboundCarCount = 0; // Reset count *before* releasing
              Synch.mutex.release();

              for (int i = 0; i < numCarsToRelease; i++) {
                  Synch.southboundSemaphore.release();
              }
              Synch.timeSim.doSleep(T);

              Synch.mutex.acquire();
              System.out.println("At time " + Synch.timeSim.curTime() + " Southbound light is turning RED.");
              Synch.currentLight = Synch.RED;
              Synch.mutex.release();
              Synch.timeSim.doSleep(Q);

              // Northbound green
              Synch.mutex.acquire();
              Synch.currentLight = Synch.NORTHBOUND_GREEN;
              System.out.println("At time " + Synch.timeSim.curTime() + " Northbound light is GREEN.");
              numCarsToRelease = Synch.northboundCarCount;
              Synch.northboundCarCount = 0;
              Synch.mutex.release();

              for (int i = 0; i < numCarsToRelease; i++) {
                  Synch.northboundSemaphore.release();
              }
              Synch.timeSim.doSleep(T);

              Synch.mutex.acquire();
              System.out.println("At time " + Synch.timeSim.curTime() + " Northbound light is turning RED.");
              Synch.currentLight = Synch.RED;
              Synch.mutex.release();
              Synch.timeSim.doSleep(Q);
          }
      } catch (InterruptedException e) {
          System.out.println("Traffic light thread interrupted: " + e);
      }
