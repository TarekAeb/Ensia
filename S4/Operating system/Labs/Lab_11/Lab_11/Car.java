...
public class Car extends Thread {
  ...
  public void run () {
    for (int I=1;  I<= 4; I++) {
      ...
      // Simulate driving around Sidi AbdAllah.
      System.out.println("At time " + Synch.timeSim.curTime() + " Car "+ myName + " is driving around Sidi AbdAllah.\n");
      Synch.timeSim.doSleep(1, 500);

      // Now cross the causeway northbound, into Zeralda.  This might
      // involve some waiting (if the northbound light is red).
      System.out.println("At time " + Synch.timeSim.curTime() + " Car "+ myName + " wants to cross southbound.\n");

      // *** Put synchronization code here, to make cars wait if the southbound
      // *** light is red.
      try{
        if (Thread.interrupted())  // Clears interrupted status!
    throw new InterruptedException();
        Synch.mutex.acquire(); // aquire mutex before checking the current light
        boolean canCross = (Synch.currentLight == Synch.SOUTHBOUND_GREEN);

        if (!canCross) {
          Synch.southboundCarCount++;
      }
      Synch.mutex.release();

      if (!canCross) {
          Synch.southboundSemaphore.acquire(); // Wait outside the mutex
      }

        // we can cross the road now
        System.out.println("At time: "+Synch.timeSim.curTime()+" Car " +myName+"is starting to cross southbound.\n");
        Synch.timeSim.doSleep(100);
      }
      catch(InterruptedException e){
          System.out.println("Car " + myName+ " interrupted while crossing southbound: "+ e);
      }
	  ...
	  // Full marks will be awarded for detailed simulation.
      ...
      // Now cross the causeway northbound, back into Sidi AbdAllah.
      System.out.println("At time " + Synch.timeSim.curTime() + " Car "+ myName + " wants to cross northbound.\n");

      // *** Put synchronization code here, to make cars wait if the northbound
      // *** light is red..
      try{
        if (Thread.interrupted())  // Clears interrupted status!
    throw new InterruptedException();
        Synch.mutex.acquire();
        boolean canCross = (Synch.currentLight == Synch.NORTHBOUND_GREEN);
        if (!canCross) {
            Synch.northboundCarCount++;
        }
        Synch.mutex.release();

        if (!canCross) {
            Synch.northboundSemaphore.acquire(); // Wait outside the mutex
        }
        System.out.println("At time " + Synch.timeSim.curTime() + " Car "+ myName + " is starting to cross northbound.\n");
        Synch.timeSim.doSleep(100);
      }
      catch (InterruptedException e) {
        System.out.println("Car " + myName + " interrupted while crossing northbound: " + e);
      }
